...
# import datafile
Smallset = read.csv("social_conflict_in_ccs_v6_Counterframe_experiments_table.csv", header = TRUE, sep = ",", skip = 6, check.names = FALSE)
...
#Plotting stuff smooth plots

ggplot(Smallset) + 
  stat_smooth(aes(x=tick,y=mean_attitude_of_citizens,group=Threshold_counter_vba,colour=Threshold_counter_vba)) + 
  facet_grid(Difference_in_vba_value1 + Difference_in_vba_value2~Difference_in_vba_value3 + Relative_power_counter_vba,labeller=label_both)
...
ggplot(Smallset) + 
  stat_smooth(aes(x=tick,y=median_attitude_of_citizens,group=Threshold_counter_vba,colour=Threshold_counter_vba)) + 
  facet_grid(Difference_in_vba_value1 + Difference_in_vba_value2~Difference_in_vba_value3 + Relative_power_counter_vba,labeller=label_both)

ggplot(Smallset) + 
  stat_smooth(aes(x=tick,y=average_euclidean_distance_average_citizens_vs_project_leaders,group=Threshold_counter_vba,colour=Threshold_counter_vba)) + 
  facet_grid(Start_accept + Xaccept ~Start_reject + Xreject + Relative_power_counter_vba,labeller=label_both)

ggplot(Smallset) + 
  stat_smooth(aes(x=tick,y=average_euclidean_distance_average_citizens_vs_project_leaders,group=Threshold_counter_vba,colour=Threshold_counter_vba)) + 
  facet_grid(Difference_in_vba_value1 + Difference_in_vba_value2~Difference_in_vba_value3,labeller=label_both)
...
ggplot(aes(x = Attitude_scales, y = Number_of_citizens_attitude), data = Endpointmelt) + 
  stat_summary(fun.y = "mean", geom = "bar", position = "dodge") + facet_grid(Threshold_counter_vba ~ Relative_power_counter_vba,labeller=label_both)
